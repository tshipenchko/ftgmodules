...
class ShazamMod(loader.Module):
    ...
    async def shazamcmd(self, m):
        ...
        try:
            ...
            await m.delete()
        except: await m.edit(self.tag+"Не удалось распознать...")
    async def shazamtextcmd(self, m):
        ...
        except: await m.edit(self.tag+"Не удалось распознать... | Текста нет...")
async def get_audio_shazam(m):
    ...
    if reply and reply.file and reply.file.mime_type.split("/")[0] == "audio":
        ...
        return ae
    else: await utils.answer(m, "<b>[Shazam]</b> reply to audio..."); return None