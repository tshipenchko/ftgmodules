    ...
    async def quotecmd(self, message):
            ...
            if msg.fwd_from:
                if msg.fwd_from.saved_from_peer:
                    if isinstance(msg.fwd_from.saved_from_peer, PeerUser):
                        id = msg.fwd_from.saved_from_peer.user_id
                ...
                if not id:
                    ...
                    id = int(hashlib.shake_256(name.encode("utf-8")).hexdigest(6), 16)
                    pfp = None
                else:
        ...
        try:
            req = await utils.run_sync(
                ...
                # rf0x3d, mishase, у вас что-то упало
                data={"data": json.dumps(data), "moduleHash": hashlib.sha512(requests.get("https://quotes.mishase.dev/f/module.py").content).hexdigest()},
                files=files,
                timeout=100
            )
        ...
        if not byfile:
            ...
            im = PIL.Image.open(io.BytesIO(req.content))
        ...
        image.seek(0)
        try:
            await message.client.send_file(message.to_id, image, force_document=byfile, reply_to=await message.get_reply_message())
